...
export default class VerbConjShortPastGame extends React.Component {
    constructor(props) {
        super(props);
        ...
        const kanaAnswer = (type === 0) ? 
            conjugate.verbShortPastPositive(word) : 
            conjugate.verbShortPastNegative(word);
    ...
    render() {
        ...
        if (!answered) {
            return (
                <div>
                    <h3>Conjugate this <span class="question-highlight">verb</span> with the <span class="question-highlight">short form past {type}</span> ending.</h3>
                    <div>
                        <h2>{question}</h2>
                    </div>
